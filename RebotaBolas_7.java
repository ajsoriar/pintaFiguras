...
class MyJPanel extends JPanel implements Runnable {
    ...
    public MyJPanel() {
        ...
        bolas = new Bola[100];
        for (int i=0;i <100; i++ ){
            int nx = (int)(Math.random()*640);
            int ny = (int)(Math.random()*480);
            int nr = (int)(Math.random()*20);
            int nv = (int)(Math.random()*10);
            int na = (int)(Math.random()*360);
            bolas[i] = new Bola(nx,ny,nr,nv,na);
        }
